...
COMMENT ON COLUMN "REPORT_BOARD".REPORT_WRITE_DATE IS '신고 일자';
...
LEFT JOIN    BOARD_IMG I ON (F.BOARD_NO = I.BOARD_NO);
...
ALTER TABLE "FLEA_BOARD"
    ADD "IMG_ORDER" NUMBER NULL;

ALTER TABLE "FLEA_BOARD"
    ADD "IMG_NO" NUMBER  NULL;

COMMENT ON COLUMN "FLEA_BOARD"."IMG_ORDER" IS '이미지 순서';
COMMENT ON COLUMN "FLEA_BOARD"."IMG_NO" IS '이미지 번호(SEQ_IMG_NO)';

ALTER TABLE "FLEA_BOARD"
    ADD CONSTRAINT "FK_FLEA_BOARD_TO_BOARD_IMG" FOREIGN KEY ("IMG_NO")
        REFERENCES "BOARD_IMG" ("IMG_NO");


SELECT * FROM BOARD ORDER BY BOARD_NO DESC;
SELECT  * FROM BOARD_COMMENT ORDER BY COMMENT_NO DESC ;
SELECT COUNT(*) FROM BOARD_COMMENT; WHERE MEMBER_NO = 3; ORDER BY COMMENT_NO DESC;
...
BEGIN
    FOR I IN 1..4 LOOP
            FOR rec IN (SELECT BOARD_NO FROM BOARD WHERE BOARD_NO > 1000) LOOP
                    INSERT INTO BOARD_COMMENT (COMMENT_NO, COMMENT_CONTENT, BOARD_NO, MEMBER_NO, PARENT_COMMENT_NO)
                    VALUES (SEQ_COMMENT_NO.NEXTVAL,SEQ_COMMENT_NO.CURRVAL || '번째 댓글입니다.', rec.BOARD_NO, FLOOR(DBMS_RANDOM.VALUE(0, 8)), NULL);
                END LOOP;
        END LOOP;
END;
...
DELETE FROM FLEA_BOARD WHERE BOARD_NO < 800;
DELETE FROM BOARD_COMMENT WHERE BOARD_NO < 800;
DELETE FROM BOARD_LIKE WHERE BOARD_NO < 800;
DELETE FROM CHATTING_ROOM WHERE BOARD_NO < 800;

DELETE FROM BOARD WHERE BOARD_NO < 800;
