...
$(document).ready(function() {
  ...
  $(document).on('click', '.update-comment-btn', function() {
      ...
      commentRow.find('.comment-content').replaceWith(`
          <textarea class="edit-comment-content" style="resize:none; width: 100%; height: 100px;">${commentContent}</textarea>
      `);
  ...
  $(document).on('click', '.cancel-edit-btn', function() {
    ...
    $(document).on('click', '#comment-report', function() {
        ...
        if (typeof commentNo === 'undefined') {
            alert("댓글 번호를 찾을 수 없습니다.");
            return;
        }
    
        if (typeof loginMemberNo === 'undefined' || loginMemberNo === null) {
            alert("로그인 한 유저만 신고가 가능합니다");
            return;
        }
    
        // 신고 팝업 열기
        window.open('/reportComment/reportPopup?commentNo=' + commentNo, '댓글 신고', 'width=500,height=600');
    });
